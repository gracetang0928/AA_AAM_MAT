...
:Usage:
	createRefData(self,PMListView,PMDetail,"id-pmschedule",data,msgCreated)
"""
def updateRefData(self,listClass,detailClass,portletName,data,msgUpdate):
	dataList = listClass(self.uidriver,portletName)
	dataList.selectFirstRecordInList()

	# Wait for load over
	detail = detailClass(self.uidriver,self.portlet)
...
def deleteRefData(self,listViewClass,portlet,formClass):
	...
	form = formClass(self.uidriver,self.portlet)
	form.click(form.backArrow)

	dataList.switchToCurrentPortletForm(self.portlet+"List")
